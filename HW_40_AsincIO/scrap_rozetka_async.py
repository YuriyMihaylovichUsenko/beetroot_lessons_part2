...
async def detail_worker(queue, worker_number):
    while True:
        ...
        try:
            async with httpx.AsyncClient() as session:                         # Changed
...
async def links_worker(queue: asyncio.Queue, worker_number: int):
    while True:
        ...
        try:
            ...
            notebooks_urls = get_notebooks_urls(response.text)[:5]                         # Changed
            print('Urls appended')
            ...
            for worker in range(len(notebooks_urls)):
                ...
                task = detail_worker(url_list_queue, worker)                   # Changed
                # task = asyncio.create_task(detail_worker(url_list_queue, worker))                   # Changed)
                tasks.append(task)
                # background_tasks.add(task)
                # task.add_done_callback(background_tasks.discard)
            # await detail_worker(url_list_queue, 111)

            print('len tasks links_worker', len(tasks))                    # Changed
            await asyncio.gather(*tasks)                    # Changed
...
async def main():
    ...
    for worker_number in range(workers):
        ...
        #     links_worker(queue, worker_number))                   # Changed
        tasks.append(task)

    print('len tasks', len(tasks))

    await asyncio.gather(*tasks)
    # for task in tasks:                         # Changed
    #     await task
...
if __name__ == '__main__':
    ...
    # print('WRITING DATA TO FILE!!!')
    # with open('rozetka.csv', 'w', newline='', encoding='utf-8') as csvfile:                         # Changed
    #     writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    #     for row in notebooks_list:
    #         writer.writerow(row)